...
import {ComposableCoW} from "lib/cow-amm/lib/composable-cow/src/ComposableCoW.sol";
import {GPv2Settlement} from "lib/cow-amm/lib/composable-cow/lib/cowprotocol/src/contracts/GPv2Settlement.sol";
import {Safe, Enum} from "lib/cow-amm/lib/composable-cow/lib/safe/contracts/Safe.sol";
import {ExtensibleFallbackHandler} from
    "lib/cow-amm/lib/composable-cow/lib/safe/contracts/handler/ExtensibleFallbackHandler.sol";
import {ConstantProduct, IPriceOracle, IERC20} from "lib/cow-amm/src/ConstantProduct.sol";
import {IConditionalOrder} from "lib/cow-amm/lib/composable-cow/src/BaseConditionalOrder.sol";
...
contract CowAmmModuleFactory {
     ...
     */
    constructor(address _settler, address _extensibleFallbackHandler, address _composableCow, address _ammHandler) {
        // We create the implementation of the module here so we can pass in the addresses of the
        // contracts that are specific to the network that the module is being deployed on. This
        // allows us to trust that the implementation for the proxy is correct, because this is
        // the contract that's deploying it!
        IMPLEMENTATION = new CowAmmModule(
            GPv2Settlement(payable(_settler)),
            ExtensibleFallbackHandler(_extensibleFallbackHandler),
            ComposableCoW(_composableCow),
            IConditionalOrder(_ammHandler)
        );
    }
...
contract CowAmmModule {
    ...
    constructor(
        ...
        COW_DOMAIN_SEPARATOR = _settler.domainSeparator();
        VAULT_RELAYER = address(_settler.vaultRelayer());
